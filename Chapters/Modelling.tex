...
\begin{lstlisting}[language=AADL,caption={TODO},label=lst:message-example]
package std_msgs
public
	data String
		properties
...
\begin{lstlisting}[language=AADL,caption={TODO},label=lst:legacy-nodes]
...
\begin{lstlisting}[language=AADL,caption={TODO},label=lst:callback-proto]
...
\begin{lstlisting}[language=AADL,caption={TODO},label=lst:impl-proto]
process implementation complex_node.impl extends ros::node.impl
subcomponents
	subscriber: thread ros::callback.impl (message => data std_msgs::String);
	publisher: thread ros::publisher.impl (message => data std_msgs::String);
	sub_pub: thread ros::call_pub.impl
		(message_in => data geometry_msgs::Pose,
		message_out => data geometry_msgs::Pose);
connections
	pub_in: port msg_in -> subscriber.msg;
	sub_out: port msg_in -> subscriber.msg;
	sub_pub_in: port msg_in -> subscriber.msg;
	sub_pub_out: port msg_in -> subscriber.msg;
properties
...
\begin{lstlisting}[float,language=ASN1,caption={ROS message, service and action definition using ASN.1},label=lst:asn1-ros]
...
\begin{lstlisting}[float,language=ASN1,caption={Internal state of a node modelled using ASN.1},label=lst:asn1-ros-is]
...
\begin{lstlisting}[float,language=ASN1,caption={TODO},label=lst:asn1-ros-is1]
params Parameters ::= {
	size 2, 
